...
public class ObiOne {
   ...
   public ObiOne(String domainLdap, String domainRoot, String domainUser, String domainPassword, String gcLdap, String gcUser,
	   String gcPassword) throws ObiOneException, ObiSchemaAccessException, ObiConnectionException, ObiNamingException {
	...
	try {
	   ...
	   if (gcLdap != null && gcLdap.isEmpty() == false) {
		...
		server.addConnectionParameters(ALIAS_SCHEMA_RACVIDE, domainLdap, new JediPath(new String[] { "" }), jediPathDomainUser,
			domainPassword);
		server.addConnectionParameters(ALIAS_GLOBAL_CATALOG, gcLdap, new JediPath(new String[] { "" }), new JediPath(gcUser),
			gcPassword);

		// Recuperation du rootDse
		List<String> listDSE = getListDse();

		if (listDSE != null && listDSE.isEmpty() == false) {
	   ...
	   throw new ObiOneException(
		   "OBIOne : OBIOne(String, String, JediPath, String, String, JediPath, String) : Echec de création des connexions");
	}
    ...
    * @todo : deprecated
    */
   protected void fillGcAttributeList(List<String> listDse) throws ObiOneException, ObiSchemaAccessException, ObiConnectionException,
	   ObiNamingException {
	List<JediObject> listResult = new ArrayList<JediObject>();
