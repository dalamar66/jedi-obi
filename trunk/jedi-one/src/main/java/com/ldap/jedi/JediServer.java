...
public class JediServer implements JediContextProvider {
	...
	public JediConnection getJediConnection(String alias) throws JediException, JediConnectionException {
		...
		try {
			...
			throw new JediConnectionException("JediServer : getJediConnection(String) : Impossible de se connecter");
		}
	}
	 ...
	 * @throws JediException
	 *             Si l'alias est null ou vide, ou si ldap est null ou vide, ou si le rootPath est null, ou si le user est null ou vide, ou si le password est
	 *             null ou vide.
	 */
	public void addConnectionParameters(String alias, String ldap, JediPath rootPath, JediPath user, String password) throws JediException {
		...
		// Verification de la validite des parametres de la methode
		if (alias == null || alias.length() == 0 || ldap == null || ldap.length() == 0 || rootPath == null || user == null || user.getPathSize() == 0) {
			...
			throw new JediException("JediServer : addConnectionParameters(String, String, JediPath, JediPath, String) : paramètres d'initialisation incorrects");
		}
		...
		connectionParameters.put("java.naming.ldap.attributes.binary", "objectGUID objectSid thumbnailPhoto oMObjectClass schemaIDGUID attributeSecurityGUID");
	...
	/**
	 * Méthode qui construit une entrée dans la base Ldap en ayant le dn , l'attribut obligatoire objectClass et la liste des attributs de l'objet
	 * 
	 * @param alias
	 *            Alias de la connexion.
	 * @param jediPath
	 ...
	 *             Si il y a un problème de connexion.
	 */
	public JediObject createLdapEntry(String alias, JediPath jediPath, JediAttribute objectClass, Attributes attributeList) throws JediException, JediConnectionException {
		// Verification de la validite des parametres de la methode
		if (alias == null || alias.length() == 0 || jediPath == null || jediPath.getPathSize() == 0 || objectClass == null || attributeList == null) {
			List<String> paramList = new ArrayList<String>();
			paramList.add(alias);
			...
			throw new JediException("JediServer : createLdapEntry(String, JediPath, JediAttribute, Attributes) : Paramètres d'initialisations incorrects");
		}
	 ...
	 *             Si il y a un problème de connexion.
	 */
	public JediObject createLdapEntry(String alias, JediPath jediPath, Attributes attributeList) throws JediException, JediConnectionException {
		try {
		...
		try {
			...
			// Recherche paginée permettant de dépasser les limites du serveur
				...
				while ((cookie != null) && (cookie.length != 0) && !exit) {
					...
					dirContext.setRequestControls(new Control[] { new javax.naming.ldap.PagedResultsControl(pageSize, cookie, Control.CRITICAL) });
				}
			}
		} catch (Exception e) {
	 ...
	 * @throws JediException
	 */
	private boolean populateSet(Integer filterType, JediFilter jediFilter, SearchControls control, int counter, InitialLdapContext dirContext,
			Set<JediObject> jediObjectSet) throws JediException {
	...
	private boolean checkJediFilter(JediFilter jediFilter) throws JediException, JediConnectionException {
		...
		if (guid != null) {
			...
			if (approximation != null || attribute != null || dn != null || filter != null || indice != null || limitation != null || metric != null
					|| pageSize != null || sorted != null || subtree != null) {
				return false;
			}
		}
		// Si on fait une recherche par dn
			...
			// ... tous les autres attributs ne doivent pas etre renseignés sauf la liste des attributs
			if (approximation != null || attribute != null || filter != null || guid != null || indice != null || limitation != null || metric != null
					|| pageSize != null || path != null || sorted != null || subtree != null) {
				return false;
			}
		}
	...
	/**
	 * Methode de factory : crée une instance de JediAttribute avec un attribut mono-valué. La stratégie de log correctement renseignée. Si la valeur de
	...
	/**
	 * Methode de factory : crée une instance de JediAttribute n'initialisant pas la valeur de l'attribut. La stratégie de log correctement renseignée.
	 * 
	 * @param nameAttribute
	 *            Nom de l'attribut.
	 * @return Instance de JediAttribute créée
	...
	/**
	 * Methode de factory : crée une instance de JediAttribute ayant une valeur mono-valué binaire. La stratégie de log correctement renseignée.
	 * 
	 * @param nameAttribute
	 *            Nom de l'attribut.
	 * @param valueAttribute
	...
	/**
	 * Methode de factory : crée une instance de JediAttribute ayant une valeur multivaluée. La stratégie de log correctement renseignée. Si une des valeurs de
	 * l'attribut multi-valué est null ou vide, elle ne sera pas ajoutée à la liste des valeurs. Si toutes les valeurs sont null ou vide alors la valeur de
